...
/// -----
/// TODO: Máy Phát Âm Thanh V2
/// -----
class MAYPHATAMTHANHV2 with CauTrucThucThiCoBan {
  ...
  Future<void> onSetupRoot() async {
    await caiDatMayPhatAmThanh(
      value:
          AudioPlayer(playerId: getMaDinhDanh ?? '[MAY_PHAT_AM_THANH_V2]')
  ...
  MAYPHATAMTHANHV2({required String maDinhDanh}) {
  ...
  Future<void> onLoop() async {
    ...
    if (getThoiLuongPhatAmThanh?.getThoiLuong == 0 && await getTrangThaiSanSangPhatAmThanh?.isDangPhatAmThanh() == true) {
      ...
      if (getMayPhatAmThanh?.state == PlayerState.playing) {
        if (kDebugMode) {
          print('Dừng Play Âm Thanh [$getMaDinhDanh]');
        }
        await getMayPhatAmThanh?.stop();
      }
    }
